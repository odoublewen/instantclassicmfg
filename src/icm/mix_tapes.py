...
class MixTapeBuilder:
    ...
    def _find_spotify_track(self, track, artist):
        ...
        if results and results.get("tracks") and results["tracks"].get("items"):
            ...
            LOG.info(
                "Found track: {name} -- {artists_str} <id:{id}>".format(artists_str=artists_str, **result)
            )
    ...
    def _process_mix_tape(self, tape, min_hit_rate, dryrun):
        ...
        if len(spotify_tracks) < len(tracks) * min_hit_rate:
            LOG.warning(
                f"Aborting playlist creation because only {len(spotify_tracks)} of {len(tracks)} tracks found"
            )
        ...
        if dryrun:
            ...
            if playlist and "id" in playlist:
                ...
                LOG.error(f"Failed to create playlist '{new_playlist_name}' for user {self.username}")

    def process_tape_list(self, tape_list, min_hit_rate, dryrun):
        for tape in tape_list:
            if "spotify_playlist" in tape:
                LOG.info(
                    "Skipping mix tape {id}: {title} (already has playlist {spotify_playlist})".format(**tape)
                )
                continue
